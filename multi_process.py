...
def process(param, global_e, local_e, test_file):
    ...
    if(param[SUITE]=='smallcrush'):
        os.system(f'Tests/testu01 -m small_crush -i {test_file} -t {param[ID]} >> {result_dest}/small_crush_{param[ID]}.txt')
    elif(param[SUITE]=='crush'):
        os.system(f'Tests/testu01 -m crush -i {test_file} -t {param[ID]} >> {result_dest}/crush_{param[ID]}.txt')
    elif(param[SUITE]=='bigcrush'):
        os.system(f'Tests/testu01 -m big_crush -i {test_file} -t {param[ID]} >> {result_dest}/big_crush_{param[ID]}.txt')
    elif(param[SUITE]=='dieharder'):
        os.system(f'Tests/dieharder -d {param[ID]} -f {test_file} -g 201 > {result_dest}/dieharder_{param[ID]}.txt')
    elif(param[SUITE]=='nist'):
        os.system(f'mkdir -p {result_dest}/nist/{param[NAME]}/')
        os.system(f'Tests/nist {param[ID]} {test_file} {result_dest}')
    else:
    ...
    while tests_completed < total_tests_all:
        ...
        for i in range(cores):
            ...
            if(process_event[i].is_set() and test_process[i] < total_tests_each_p[i]):
                ...
                test_file = f'random_numbers_time/{process_list[i][test_process[i]][SUITE]}_{process_list[i][test_process[i]][NAME]}_{process_list[i][test_process[i]][ID]}.bin'
                with open(random_source,'rb') as rf, open(test_file,'wb') as wf:
                    #wf.seek(file_seek, 0)
                    file_size = min(int(Tests[tests_completed][SIZE]), 5_400_000_000)
    ...
    while tests_completed < total_tests_all:
        ...
        for i in range(cores):
            ...
            if(process_event[i].is_set() and tests_completed < total_tests_all):
                ...
                #TODO: test file for NIST update
                test_file = f'random_numbers_time/{Tests[tests_completed][SUITE]}_{Tests[tests_completed][NAME]}_{Tests[tests_completed][ID]}.bin'
                with open(random_source,'rb') as rf, open(test_file,'wb') as wf:
                    #wf.seek(file_seek, 0)
                    file_size = min(int(Tests[tests_completed][SIZE]), 5_400_000_000)
                    wf.write(rf.read(file_size))
